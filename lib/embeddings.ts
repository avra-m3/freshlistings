...
export const mapEmbeddings = async<K extends string> (texts: Record<K, string>): Promise<Record<string, number[]>> => {
    const keys = Object.keys(texts)
    const values = keys.map(k => texts[k])
    return (await embeddings.embedDocuments(values)).reduce((acc, embedding, i) => {
